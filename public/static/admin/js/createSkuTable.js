...
//			});
//
        SKUTableDom += "<table class='table table-border table-bordered table-hover table-bg mt-20'><thead><tr class='text-c'>";
        //创建表头
        for(var t = 0 ; t < skuTypeArr.length ; t ++){
            SKUTableDom += '<th>'+skuTypeArr[t].skuTypeTitle+'</th>';
        }
        SKUTableDom += '<th>价格</th><th>库存</th>';
        SKUTableDom += "</tr></thead>";
        //循环处理表体
...
//				});
            ...
            if(alreadySetSkuVals){
                ...
                if(currGroupSkuVal){
                    alreadySetSkuPrice = currGroupSkuVal.attr_price;
                    alreadySetSkuStock = currGroupSkuVal.attr_gold
                }
            }
...
function getAlreadySetSkuVals(){
	...
	$("tr[class*='sku_table_tr']").each(function(){
        ...
        if(skuPrice || skuStock){//已经设置了全部或部分值
			...
			var propvalnames = $(this).attr("propvalnames");//SKU值主键名集合
			alreadySetSkuVals[propvalids] = {
				'attr_id':propvalids,
				'attr_name':propvalnames,
				"attr_price" : skuPrice,
				"attr_gold" : skuStock
			}
		}else{
            layer.msg('请填写价格和金币',{icon:5,time:1000})
        }
	});
}
